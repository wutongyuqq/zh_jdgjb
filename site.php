...
class Zh_jdgjbModuleSite extends Core {
	...
	public function doMobileSelectUser(){
		global $_W, $_GPC;
		$sql =" select id,name from ".tablename('cjdc_user')." where uniacid={$_W['uniacid']}  and  openid like '%{$_GPC['keywords']}%'";
		echo json_encode(pdo_fetchall($sql));
	}
   ...
   //删除充值活动
	...
	public function doMobileSelectUser2(){
		global $_W, $_GPC;
		$sql =" select id,name from ".tablename('cjdc_user')." where uniacid={$_W['uniacid']}  and  openid like '%{$_GPC['keywords']}%' and id not in (select user_id from".tablename('zh_jdgjb_seller')." where  uniacid={$_W['uniacid']})";
		echo json_encode(pdo_fetchall($sql));
	}
   ...
   public function doMobileGetYj(){
        ...
        $sql=" select b.name as yj_name,c.name as fx_name,a.time from".tablename('zh_jdgjb_fxuser')." a left join ".tablename('cjdc_user')." b on a.fx_user=b.id left join ".tablename('cjdc_user')." c on a.user_id=c.id where a.user_id={$user_id}";
...
//获取二级
   public function doMobileGetEj(){
        ...
        foreach ($res as $key => $value) {
              $sql=" select b.name as yj_name,a.time from".tablename('zh_jdgjb_fxuser')." a left join ".tablename('cjdc_user')." b on a.fx_user=b.id  where a.user_id={$value['fx_user']}";
