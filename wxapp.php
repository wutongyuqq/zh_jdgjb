...
class Zh_jdgjbModuleWxapp extends WeModuleWxapp {
    ...
    /*
     * 更新分销数据，如果酒店分享二维码给其他人成为上下级关系，则默认在其他小程序也为上下级关系
     * 宝塔每天都执行该任务
     */
    public function doPageUpdateFxData(){
        global $_GPC, $_W;
        // $res = pdo_getall('zh_jdgjb_', array('uniacid' => $_W['uniacid'], 'status' => 1), array(), '', 'orderby asc');
        //  $sql = "select *  from " . tablename("zh_jdgjb_fxuser") . " left join zh_jdgjb_user on ims_zh_jdgjb_fxuser.  order by id  desc ";

        $sql = "select a.* ,b.tel from " . tablename("zh_jdgjb_fxuser") . " a" . " left join " . tablename("cjdc_user") . " b on b.id=a.user_id where b.tel  IS NOT NULL AND b.tel <>'' AND  a.fx_user  IS NOT NULL AND a.fx_user <>''";
        $resArray = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
        foreach ($resArray as $key => $val) {
            $res = pdo_get('cjdc_user', array('id' => $val['fx_user']));
            //如果下级代理tel存在，则更新点餐分销规则
            if($res['tel']){
                $res2 = pdo_get('cjdc_user', array('user_tel' => $res['tel']));
                $res3 = pdo_get('cjdc_user', array('user_tel' => $val['tel']));


                $res4 = pdo_get('cjdc_fxuser', array('user_id' => $res3['id'],'fx_user' => $res2['id']));
                if($res4){

                }else {
                    $data['user_id'] = $res3['id'];
                    $data['fx_user'] = $res2['id'];
                    $data['time']=date('Y-m-d H:i:s');
                    $res5 = pdo_insert('cjdc_fxuser', $data);
    ...
    public function doPageFx_tel(){
        global $_W,$_GPC;

        $res = pdo_get('cjdc_user',array('id'=>$_GPC['user_id']));
        //更新外卖余额
        $int = pdo_update('cjdc_user',array('wallet'=>$res['wallet']),array('user_tel'=>$res['tel']));


        //更新商城余额
        $int2 = pdo_update('ewei_shop_member',array('credit2'=>$res['wallet']),array('mobile'=>$res['tel']));

        if(bccomp($res['wallet'],$int2['credit2'],2) == -1){
            echo "余额不一样";
            die;
        }

        if(bccomp($res['wallet'],$int['wallet'],2) == -1){
            echo "余额不一样";
            die;
        }
...
//解密
    ...
    //更新手机号
    public function doPageUploadPhoneInfo() {
        global $_W, $_GPC;
...
//房型列表
    ...
    public function doPageRoomList() {
        global $_GPC, $_W;
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = $_GPC['pagesize'] ?:50;
        $data[':uniacid'] = $_W['uniacid'];
        $data[':seller_id'] = $_GPC['seller_id'];
        $where = " where uniacid=:uniacid and seller_id=:seller_id and isKtvRoom=0 and state=1 ";
    ...
    public function doPageKtvRoomList() {
        global $_GPC, $_W;
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = $_GPC['pagesize'] ?:50;
        $data[':uniacid'] = $_W['uniacid'];
        $data[':seller_id'] = $_GPC['seller_id'];
        $where = " where uniacid=:uniacid and seller_id=:seller_id and isKtvRoom=1  ";
        $sql = "select *  from " . tablename("zh_jdgjb_room") . $where . " order by sort asc ";
        $select_sql = $sql . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize;
        $list = pdo_fetchall($select_sql, $data);
        echo json_encode($list);
    ...
    public function doPageIsForceGetPhone()
    {
        global $_GPC, $_W;
        $data['user_id'] = $_GPC['user_id'];
        if($data['user_id']) {
            ...
            if ($res1["user_tel"]) {
                ...
                $res = pdo_get("cjdc_system", array('id' => 1));
                //1,强制获取  0 不强制获取
                echo $res['is_tel'];
            }
        }else{
...
//使用优惠券
            ...
            $res3 = pdo_get('cjdc_user',array('id'=>$data['user_id']));
            //更新外卖余额
            ...
            //更新商城余额
            $int2 = pdo_update('ewei_shop_member',array('credit2'=>$res3['wallet']),array('mobile'=>$res3['tel']));

            if(bccomp($res3['wallet'],$int2['credit2'],2) == -1){
                echo "余额不一样";
                die;
            }
...
//保存评价
    public function doPageSaveAssess() {
        ...
        if ($res) {
            ...
            pdo_update('cjdc_user', array('score +=' => $res2['pl_score']), array('id' => $_GPC['user_id']));
        } else {
            echo '2';
        }
    }
...
//评价列表
    public function doPageAssessList() {
        ...
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = 10;
        $sql = " select a.*,b.name,b.img as logo from" . tablename('zh_jdgjb_assess') . " a left join " . tablename('cjdc_user') . " b on a.user_id=b.id  where a.uniacid=:uniacid  and a.seller_id=:seller_id order by a.id desc";
        //echo $sql;die;
        $select_sql = $sql . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize;
...
//打印机
    public function doPagePrint() {
        ...
        $roomObj = pdo_get('zh_jdgjb_room', array('id' => $orderInfo['room_id']));
        $isKtvRoom = $roomObj['isKtvRoom'];
...
//打印机开启
            if ($dyj['type'] == 3) {
                ...
                if($isKtvRoom==1) {
                    ...
                    $content .= "KTV房型: " . $orderInfo['room_type'] . "\n\n";
                    $content .= "房间价格: " . $orderInfo['dis_cost'] . "\n\n";
                    $content .= "押金: " . $orderInfo['yj_cost'] . "\n\n";
                    $content .= "优惠金额: " . $yh_money . "\n\n";
                    $content .= "房间数量: " . $orderInfo['num'] . "\n\n";
                    $content .= "预定日期: " . substr($orderInfo['arrival_time'], 0, 10) . "\n\n";
                    $content .= "预定人姓名: " . $orderInfo['name'] . "\n\n";
                    $content .= "预定人电话: " . $orderInfo['tel'] . "\n\n";
                    $content .= "支付方式: " . $type . "\n";
                    $content .= "\n\n\n";
...
//365
            ...
            if ($dyj['type'] == 2) {
                if($isKtvRoom==1){
                    $dyj = pdo_get('zh_jdgjb_dyj', array('seller_id' => $orderInfo['seller_id'],'id'=>2));
                    $rst = Dyj::ylydy($dyj['api'], $dyj['token'], $dyj['yy_id'], $dyj['mid'], $content);
                }else {
//易联云
                    $dyj = pdo_get('zh_jdgjb_dyj', array('seller_id' => $orderInfo['seller_id'],'id'=>1));
                    $rst = Dyj::ylydy($dyj['api'], $dyj['token'], $dyj['yy_id'], $dyj['mid'], $content);
                }
            }
            if ($dyj['type'] == 3) {
...
//我的订单
    public function doPageMyOrder() {
        ...
        $data[':uniacid'] = $_W['uniacid'];
        $data[':user_id'] = $_GPC['user_id'];
        $where = " where ims_zh_jdgjb_order.uniacid=:uniacid and ims_zh_jdgjb_order.user_id=:user_id  ";
        if ($_GPC['status']) {
            $where .= " and ims_zh_jdgjb_order.status=:status";
            $data[':status'] = $_GPC['status'];
        }

        //$sql = "select ". tablename(zh_jdgjb_order).".*,".tablename(zh_jdgjb_room).".isKtvRoom from ".tablename(zh_jdgjb_order)." LEFT JOIN ".tablename(zh_jdgjb_room)." on ".tablename(zh_jdgjb_order).".room_id = ".tablename(zh_jdgjb_room).".id ". $where . " order by ".tablename(zh_jdgjb_order).".id desc ";

        $sql = "SELECT ims_zh_jdgjb_order.*,ims_zh_jdgjb_room.isKtvRoom FROM ims_zh_jdgjb_order LEFT JOIN ims_zh_jdgjb_room on ims_zh_jdgjb_order.room_id = ims_zh_jdgjb_room.id " . $where . " order by ims_zh_jdgjb_order.id desc ";
...
//虚拟红包
            ...
            pdo_update('cjdc_user', array('score -=' => $_GPC['integral']), array('id' => $_GPC['user_id']));
            echo '1';
        } else {
            echo '2';
        }
...
//获取实时房间数量
    ...
    public function doPageFaceToFace() {
        ...
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = 10;
        $sql = "select a.*,b.name,b.img from (select * from ims_cjdc_qbmx where time like '".$selectDate."%' and note='当面付订单') a LEFT join ims_cjdc_user b on a.user_id = b.id order by time desc";
        $select_sql = $sql . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize;
    ...
    //当面付订单明细
    public function doPageGetPayList() {
        global $_W, $_GPC;
        $selectDate = $_GPC['selectDate'];
        $pageindex = max(1, intval($_GPC['page']));
        $pagesize = 10;
...
//申请分销商
    ...
    public function doPageMySx() {
        global $_W, $_GPC;
        $sql = "select a.* ,b.name from " . tablename("zh_jdgjb_fxuser") . " a" . " left join " . tablename("cjdc_user") . " b on b.id=a.user_id   WHERE a.fx_user=:fx_user ";
...
//绑定分销商
    public function doPageBinding() {
        ...
        if ($_GPC['user_id'] == $_GPC['fx_user']) {
            ...
            //修改数据库$res3 = pdo_pudate('zh_jdgjb_fxuser',array('mobile'=>$_GPC['13812345678']),array('id'=>'2'));
            //删除数据库$res3 = pdo_delete('zh_jdgjb_fxuser',array('id'=>'3'));
            $res3 = pdo_insert('zh_jdgjb_fxuser', array('user_id' => $_GPC['user_id'], 'fx_user' => $_GPC['fx_user'],'mobile'=>$_GPC['mobile'], 'time' => time()));
            if ($res3) {
                echo '1';
            } else {
                echo '2';
...
//查看我的团队
    public function doPageMyTeam() {
        global $_W, $_GPC;
        $sql = "select a.* ,b.name ,b.img from " . tablename("zh_jdgjb_fxuser") . " a" . " left join " . tablename("cjdc_user") . " b on b.id=a.fx_user   WHERE a.user_id=:user_id order by id DESC";
        $res = pdo_fetchall($sql, array(':user_id' => $_GPC['user_id']));
        $res2 = array();
        for ($i = 0; $i < count($res); $i++) {
            $sql2 = "select a.* ,b.name ,b.img from " . tablename("zh_jdgjb_fxuser") . " a" . " left join " . tablename("cjdc_user") . " b on b.id=a.fx_user   WHERE a.user_id=:user_id order by id DESC";
...
//佣金提现
    public function doPageYjtx() {
        ...
        $sql3 = " select ifnull(sum(tx_cost),0) as tx_money from " . tablename('zh_jdgjb_commission_withdrawal') . " where  user_id=:user_id and state in (1,2)";
        $ytx_money = pdo_fetch($sql3, array(':user_id' => $_GPC['user_id']));
        $ktx = number_format($money['yx'] - $ytx_money['tx_money'], 2);
        if ($ktx >= $_GPC['tx_cost']) {
            $data['tx_cost'] = $_GPC['tx_cost'];
            $data['sj_cost'] = $_GPC['sj_cost'];
...
//完善信息
    public function doPageRenewUser() {
        ...
        $res = pdo_update('cjdc_user', $data, array('id' => $_GPC['user_id']));
        if ($res) {
            echo '1';
        } else {
            echo '2';
...
//入住改变
    public function doPageUpdRz() {
        ...
        if ($rst) {
            pdo_update('cjdc_user', array('score +=' => $score), array('id' => $order['user_id']));
        }
        if ($level_id) {
            //更改会员等级
            pdo_update('cjdc_user', array('level_id' => $level_id, 'type' => 2), array('id' => $order['user_id']));
            //echo $level_id;
        }
        if ($order['type'] == 3) {
            roomNum($_GPC['id']);
