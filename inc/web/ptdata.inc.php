...
$time4=date("Y-m");
//会员总数
$totalhy=pdo_get('cjdc_user', array('uniacid'=>$_W['uniacid']), array('count(id) as count'));
//今日新增会员
$sql=" select a.* from (select  id,FROM_UNIXTIME(join_time) as time  from".tablename('cjdc_user')." where uniacid={$_W['uniacid']}) a where time like '%{$time2}%' ";
$jr=count(pdo_fetchall($sql));
//昨日新增
$sql2=" select a.* from (select  id,FROM_UNIXTIME(join_time) as time  from".tablename('cjdc_user')." where uniacid={$_W['uniacid']}) a where time like '%{$time3}%' ";
$zuor=count(pdo_fetchall($sql2));
//本月新增
$sql3=" select a.* from (select  id,FROM_UNIXTIME(join_time) as time  from".tablename('cjdc_user')." where uniacid={$_W['uniacid']}) a where time like '%{$time4}%' ";
