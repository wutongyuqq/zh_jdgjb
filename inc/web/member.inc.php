...
$sql="select a.* ,b.name as level_name from " . tablename("cjdc_user") .  " a"  . " left join " . tablename("zh_jdgjb_level") . " b on a.level_id=b.id". $where ." order by id desc";
$total=pdo_fetchcolumn("select count(*)  from " . tablename("cjdc_user").  " a"  . " left join " . tablename("zh_jdgjb_level") . " b on a.level_id=b.id" .$where,$data);
...
if(checksubmit('submit3')){
      $res=pdo_update('cjdc_user',array('score +='=>$_GPC['reply']),array('id'=>$_GPC['id3']));
      if($res){
...
if(checksubmit('submit2')){
      ...
      if($res3){
      pdo_update('cjdc_user',array('wallet +='=>$_GPC['cz_money']),array('id'=>$_GPC['id2']));
       message('充值成功！', $this->createWebUrl('member'), 'success');
      }else{
       message('充值失败！','','error');
      }
...
if(checksubmit('submit4')){
      global $_W, $_GPC; 
      $res4=pdo_update('cjdc_user',array('level_id'=>$_GPC['level_id'],'dj_money'=>0),array('id'=>$_GPC['id4']));
      if($res4){
