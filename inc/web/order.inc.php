...
if($operation=='rz'){
	...
	//获取用户消费金额
			$userInfo=pdo_get('cjdc_user',array('id'=>$order['user_id']),array('level_id','dj_money'));
			...
			pdo_update('cjdc_user',array('dj_money +='=>$order['dis_cost']),array('id'=>$order['user_id']));
		}
		if($type['is_score']==1){
	...
	//更改会员等级
		$level=pdo_get('zh_jdgjb_level',array('id'=>$level_id),array('value'));
			pdo_update('cjdc_user',array('level_id'=>$level_id,'type'=>2,'dj_money -='=>$level['value']),array('id'=>$order['user_id']));
	//echo $level_id;
		}
		if($order['type']==3){
