        ...
        if ($_W['role'] == 'operator') {
            ...
            $navemenu[14] = array(
                ...
                'items' => array(
                    ...
                    3 => $this->createMainMenu('用户充值 ', $do, 'user2', ''),
                    4 => $this->createMainMenu('充值记录 ', $do, 'addmoneylove', ''),
                )
            );
            $navemenu[8] = array(
                'title' => '<a href="index.php?c=site&a=entry&op=display&do=hotel&m=zh_jdgjb" class="panel-title wytitle" id="yframe-8"><icon style="color:#8d8d8d;" class="fa fa-cart-plus"></icon>  酒店管理</a>',
    ...
    public function getMainMenu2()
    {
        ...
        $navemenu[12] = array(
            ...
            'items' => array(
                ...
                3 => $this->createMainMenu('充值记录 ', $do, 'addmoneylove', ''),
...
//入住模板消息
    public function rzMessage($order_id){
        ...
        function set_msg($_W,$order_id){
            ...
            $userInfo=pdo_get('cjdc_user',array('id'=>$list['user_id']),'openid');
